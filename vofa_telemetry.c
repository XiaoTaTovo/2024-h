...
{
    send_text("#ROUTE READY H2026 V50 T4-10 persistent defaults\r\n");
...
void VofaTelemetry_SendRoute(const CarFirmware *firmware, uint32_t uptimeMs)
{
    ...
    /* FireWater treats the empty token after a trailing comma as an invalid
     * float and drops the whole route frame.  The final field has no comma. */
    out = append_unsigned(out,
